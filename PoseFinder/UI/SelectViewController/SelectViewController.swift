...
class SelectViewController: UIViewController{
    ...
    func loadupperimg(){
        ...
        upperImgview = UIImageView(frame: CGRect(x: view.frame.minX,
                                                 ...
                                                 height: view.frame.height*0.005) )
    ...
    func selectExerButton(){
        ...
        correctButton = UIButton(frame: CGRect(x: view.frame.width*0.05,
                                             y: view.frame.height*0.795,
                                             width: view.frame.width*0.9,
                                             height: view.frame.height*0.15))
        //plankButton.layer.cornerRadius = 10
        correctButton.clipsToBounds = false
        correctButton.imageView?.contentMode = .scaleAspectFit
        correctButton.layer.shadowOffset = CGSize(width: 5, height: 5)
        correctButton.layer.shadowOpacity = 0.7
        correctButton.layer.shadowRadius = 5
        correctButton.layer.shadowColor = UIColor.gray.cgColor
        //correctButton.backgroundColor = .red
        correctButton.setImage(UIImage(named: "변경사진.png"), for: UIControl.State.normal)
        plankButton.contentMode = .scaleAspectFill
        correctButton.addTarget(self, action: #selector(toDetectView), for: UIControl.Event.touchUpInside)
