...
var check = 0
var Counts = Array(repeating: 0.0, count : 34*150)
var model_output: Int64!
var posture_diagnosis: String = "Do your squats"
    ...
    var poses: [Pose] {
        ...
        detectedPoses.forEach { pose in
            pose.joints.values.forEach { joint in
                if (check>=100) && (check<=249) {
                    
                    Counts[(check-100)*34 + joint.name.rawValue*2] = joint.position.y
                    Counts[(check-100)*34 + joint.name.rawValue*2+1] = joint.position.x
        ...
        if check == 250   {
            //print(Counts)
            let model : MLModel
            model = try! randomforest3(configuration: .init()).model
            //print(model)
            
            let input = try!MLMultiArray(shape: [5100], dataType: MLMultiArrayDataType.double)
            
            for (index, element) in Counts.enumerated(){
                input[index] = NSNumber(floatLiteral: element)
            }
            
            let input2 = randomforest3Input(input: input)
            
            let output = try! model.prediction(from: input2)
            
            model_output = randomforest3Output.init(features: output).classLabel
            //print("rf: ", model_output)
            //print("rf: ", randomforest3Output.init(features: output).classProbability)
            //print(output)
            
            if(model_output == 0){
                self.label4.text = "Very Good!🥳"
            }else if(model_output == 1){
                self.label4.text = "Bad back round😅"
            }else if(model_output == 2){
                self.label4.text = "깔짝충 ㅉㅉ😒"
            }
            check = 0
            Counts = Array(repeating: 0.0, count : 34*150)
        }
